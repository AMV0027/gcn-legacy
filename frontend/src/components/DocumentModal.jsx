...
function DocumentModal({ isOpen, onClose }) {
  ...
  const handleDelete = async (pdfName) => {
    if (window.confirm("Are you sure you want to delete this document?")) {
      try {
        await axios.delete(`http://localhost:5000/api/delete-pdf/${pdfName}`);
  ...
  const handleEdit = async (pdfName) => {
    try {
      await axios.put(`http://localhost:5000/api/update-pdf-info/${pdfName}`, {
        new_info: editInfo,
      });
  ...
  const handleDownload = async (pdfName) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/pdf?name=${pdfName}`,
        {
          responseType: "blob",
        }
      );
